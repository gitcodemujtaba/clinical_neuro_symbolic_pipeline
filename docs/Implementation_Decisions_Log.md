...
### Locked-test-split contamination fixed in calibrator/KGE training data; `CALIBRATED_AUTO_THRESHOLD` re-derived and bumped
- **Decision**: Fixed two scripts (`scripts/retrain_calibrator_full_corpus.py`, `scripts/build_kg_embeddings.py`'s `gather_tp_records()`) that sourced training-note pools unconditionally rather than excluding `data/splits/note_splits.csv`'s official locked test split, plus 4 more evaluation scripts sharing the same unsafe-default pattern (`evaluation/ablations.py`, `evaluation/stage_calibration.py`, `scripts/measure_gliner_risk_vs_match_tier.py`, `scripts/run_tier_gate_batch.py`). A second, related leak (fresh-5's real validation notes re-entering the calibrator's training pool) was found and fixed the same day while re-verifying the first fix. Refit the production calibrator on the doubly-clean pool, then re-derived `CALIBRATED_AUTO_THRESHOLD` on that clean model using the exact selection rule the original 0.72 was chosen with (smallest threshold reaching 100% val precision, hard traps active) — moved 0.72 → 0.78.
- **Evidence**: 39 of 149 notes (26%) feeding the contaminated calibrator fit were from the locked test split; refit on the clean 105-note pool measured val AUROC 0.868 (up from the previously-reported, contaminated 0.852) with zero training-set overlap against both fresh-10 and fresh-5. On that clean model, 0.72 measured only ~98% precision (not the 100% it was originally chosen for) — a real drift, not staleness alone — with `neck pain` (score 0.779742) as the sole remaining false positive; 0.78 is the smallest threshold clearing it, at 13.6% coverage (down from ~16-17% at 0.72 on the same pool).
- **Source**: docs/ConsensusCalibrator_Technical_Reference.md (§18, §19); docs/RotatE_KG_Embedding_Technical_Reference.md (§4, corrected numbers)
- **Date**: 2026-08-31
