...
> **Status: current as of 2026-08-20.** This doc describes the pipeline as it
...
> ~25 dated point-in-time docs in this folder. Those docs remain as the
> historical record of *how* each piece was found/fixed/measured — this doc
> is the *current-state* reference. See `docs/2026-08-20_Session_Results_And_Status.md`
> for the latest numbers and open items.
...
* **Athena/OHDSI OMOP vocabulary tables**, loaded into DuckDB
  ...
  `athena_concept_ancestor`.
* **SNOMED CT US IS_A hierarchy** — also queried via Memgraph (Bolt,
  `bolt://localhost:7688`) for KG3 write-back and Stage 3 escalation
  evidence.
...
independently run a two-step chain-of-thought (define the clinical meaning
first, with no candidate list visible; then evaluate candidates
sequentially starting at #1, stop at first accept) via `src/llm_client.py`.
...
| `TIER_1_AUTO_VALIDATED` | 3/3 unanimous SUPPORTED on candidate #1 | AUTO |
| `TIER_1B_CALIBRATED_AUTO_VALIDATED` | fails hard Tier 1/3 rules, but a fitted calibrator scores it above threshold | AUTO (separate audit trail from genuine Tier 1) |
...
| `TIER_5_TRUE_AMBIGUITY` | low top-candidate similarity / degenerate context | HITL_REQUIRED |
...
count) scoring `P(correct)` only for entities that already fail every hard
Tier 1/3 rule. `CALIBRATED_AUTO_THRESHOLD = 0.72`. Two hard "trap" gates
bypass the calibrator entirely for known-fragile patterns found via direct
false-positive investigation: `_is_coronary_segment_trap()` (LCX/LMCA-style
SapBERT embedding collapse) and `_is_short_alphanumeric_code()`
(`^[A-Za-z]{1,2}[0-9]{1,2}$`, catches S2/T1/V12-shaped collisions
...
2. **HITL Review Queue** — the human-review interface for everything not
   in `AUTO_TIERS`; the only page that opens a write connection.
...
Clinical-T5 runs as an independent baseline on the same notes, passed
through the same SapBERT/DuckDB grounding step so both are scored at the
concept-ID level rather than on raw text overlap. Compared at three
longitudinal checkpoints (KG3 empty, ~half-processed, fully processed).
