"""Objective 2 (mollm_decisions) analogue of select_anchoring_bias_cases.py --
curates a set of real False Negatives at high Stage 2 confidence where the
entity text and candidate concept name are genuinely name-divergent (real
abbreviation/acronym traps), not trivially-identical-string cases that are
mostly SNOMED-crosswalk-granularity grading artifacts (see
docs/2026-08-15_Contradiction_Detection_Analysis.md S3c/S6 for why that
distinction matters -- testing a devil's-advocate prompt fix against
artifacts it can't and shouldn't "fix" produces an uninterpretable result).

Unlike Objective 3, mollm_decisions has three modes (resolution/contradiction/
non_asserted_check) with different verdict vocabularies. `verdict` is
classified via the same classify_verdict() unified mapping used throughout
this session's analysis scripts, so mode differences don't need branching
here either.
"""
...
def classify_verdict(v):
    ...
    if v in ('CONTRADICTED', 'NONE_CORRECT', 'ENTITY_LABEL_INCORRECT',
             'CONCEPT_MAPPING_INCORRECT', 'BOTH_INCORRECT'): return 'FLAGS'
...
for entity_id, note_id, orig_start, orig_end, entity_label, original_text, cands_json, models_json in rows:
    ...
    verdicts = [m.get('verdict') for m in models]
...
with open('reports/contradiction_detection/anchoring_bias_targets_obj2.json', 'w') as fh:
